  ...
  const analyzeContent = async () => {
    ...
    const generateDynamicHallucinations = (content: string, maxCount: number) => {
      ...
      const patterns = [
        ...
        {
          regex: /(according to|research shows|studies indicate)/gi,
          type: "Unverified Source",
          explanation: "Vague reference to research without specific citation"
        },
        {
          regex: /(all experts|everyone agrees|universally accepted)/gi,
          type: "Absolute Statement",
          explanation: "Unlikely absolute consensus without evidence"
        }
      ];

      patterns.forEach(pattern => {
        let match;
        while ((match = pattern.regex.exec(content)) !== null && hallucinations.length < maxCount) {
          hallucinations.push({
            text: match[0],
            type: pattern.type,
